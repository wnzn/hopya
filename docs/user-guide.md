...
Open **Webhooks &amp; automations** from the account menu. What appears depends on your workspace role: `workspace:manage` manages workspace webhooks, `automations:manage` manages graphs and runs, and `credentials:manage` manages authentication profiles. These permissions can be delegated separately. Roles that had workspace management before the graph upgrade initially receive both new permissions, so owners should review them.

Workspace webhooks push `item.*`, `node.*` and `field.changed` events to up to 20 endpoints. A new or rotated hook shows its secret once and uses signing version 2: `x-hopya-signature` contains `sha256=` plus HMAC-SHA256 of the exact JSON body, and redirects are rejected. Older hooks retain their historical version-1 digest/redirect behavior until rotated; coordinate receiver verification before rotation.

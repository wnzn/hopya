...
The **Body** uses the TipTap rich-text editor and is stored as Markdown. Its toolbar offers undo/redo, bold, italic, strikethrough, inline code, headings, lists, quotes, code blocks and links; an existing link can be edited or removed. Plain typing stays plain Markdown. Board and list previews show plain text; control characters are normalized on save and formatting is preserved verbatim up to the 50,000-character limit.
...
On narrow screens use the navigation toggle and horizontally scroll wide boards, calendars and tables. **Tab** moves through controls; task-view tabs also support arrow keys and Home/End. Open cards with the keyboard and change statuses with **Move to** rather than dragging. Dialogs support keyboard focus and Escape to close. Use the displayed save action to persist edits before closing.

## Settings

Click your account icon at the bottom of the sidebar to open **Manage fields &amp; statuses**, **Field management**, **Import &amp; export**, **Webhooks &amp; automations**, **API docs**, Settings, Administration (site administrators only), or Sign out. The account menu supports keyboard activation and Escape to close.
...
At `/settings`, update your display name or change your password using your current password and a new password of at least 12 characters. Password changes revoke other sessions and existing personal tokens, so reconnect affected clients. Email changes are not supported in the current UI.
...
Choose a project, standalone list or a list inside a project, then **Add fields** in List or Table. The same control is available inside task details without discarding an unsaved draft. In All tasks, explicitly choose the target in the picker. Check existing catalog fields or optional built-ins and choose **Apply fields**, or use **Create field and add**. Templates include Estimate (hours), Effort (points), Progress (%) and Reference; they are ordinary typed fields, not automatic unit/range enforcement.

Core list/table columns are Task, Status, Assignee and Due date. Optional columns include Priority, Start date, Tags, Description, List, Created and Updated. Projects and standalone lists select their own optional/custom fields; lists inside projects inherit the project's choices. New field owners start without optional fields. Existing projects retain their previous field visibility after migration so saved work does not disappear unexpectedly; trim their configuration using the picker.
...
Provider credentials, `APP_URL`, the landing-page switch, registration and SSO provisioning are operator environment settings, not ordinary user preferences. A service status flag means configured, not necessarily reachable or tested.
...
Site administrators manage the public landing page and branding from `/admin`: disable the landing page (visitors go to sign-in), or upload a custom logo (PNG, JPEG, WebP or SVG up to 300 KB) shown in the navigation and sign-in areas. The landing page renders the operator-editable `apps/web/src/landing.json` template.
...
Export from the same page by workspace, scope (all tasks or one list), status, search text and format; the file downloads as `hopya-export-*.csv` or `*.json` with custom fields flattened to `custom:<Name>` columns.

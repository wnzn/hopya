...
Hopya uses the official TypeScript MCP SDK v1. The stdio process forwards to REST with `HOPYA_API_URL` (origin only) and `HOPYA_API_TOKEN`. An administrator can separately enable the SSE endpoint at `/api/v1/mcp/sse` in **Site settings**; it is disabled by default and accepts personal tokens only through the `Authorization: Bearer` header. See [README configuration](../README.md#api-and-mcp).

Default tools on both transports: `list_workspaces`, `get_workspace`, `list_items`, `get_item`. Mutation tools `create_item`, `update_item`, `delete_item` appear only with `HOPYA_MCP_ALLOW_WRITES=true` on the API process or stdio process. That is operator permission to expose tools, **not per-action human consent**. The host must enforce its own human-approval policy. Keep writes disabled for hosts that cannot do so. Tool annotations are hints, not security enforcement. All REST RBAC, validation, version conditions and token revocation remain authoritative.
...
Use a dedicated account with only required workspace memberships. Tokens currently inherit all that account's permissions rather than taking separate scopes. To avoid stale updates, pass `expectedUpdatedAt` to `update_item`. Keep tokens in the MCP client's private environment or secure header configuration, never arguments, URLs, logs, or source control. Run `node apps/api/build/bin/mcp.js` after `npm ci` and building the API. Do not wrap it with commands that print non-protocol text to stdout. SSE allows at most four sessions per user and 32 per instance; each session expires after 30 minutes and is closed when the endpoint is disabled. POST messages are reauthenticated, so token revocation and account disabling stop subsequent calls.

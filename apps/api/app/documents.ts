...
async function documentInWorkspace(wid: string, documentId: string): Promise<DocumentRecord> {
  const document = await db.get<DocumentRecord & Record<string, unknown>>('SELECT * FROM documents WHERE workspaceId=? AND id=?', wid, id.parse(documentId))
...
async function listReactions(userId: string, wid: string, documentId: string) {
  ...
  for (const row of rows) {
    ...
    reactions.push({ emoji: row.emoji, count: row.count, reactedByMe: Boolean(row.reactedByMe) })
...
export const documentService = {
  async listDocuments(userId: string, wid: string) {
    ...
    return db.all<Pick<DocumentRecord, 'id' | 'workspaceId' | 'parentId' | 'title' | 'createdAt' | 'updatedAt'>>(
      'SELECT id,workspaceId,parentId,title,createdAt,updatedAt FROM documents WHERE workspaceId=? ORDER BY createdAt,id', wid)
  },
  ...
  async createDocument(userId: string, wid: string, input: unknown) {
    ...
    return db.transaction(async () => {
      ...
      const createdAt = now()
      const document: DocumentRecord = { id: randomUUID(), workspaceId: wid, ...data, bodyRevision: 1, createdAt, updatedAt: createdAt }
      await db.run(`INSERT INTO documents(id,workspaceId,parentId,title,body,bodyRevision,createdAt,updatedAt)
        VALUES (@id,@workspaceId,@parentId,@title,@body,@bodyRevision,@createdAt,@updatedAt)`, document)
  ...
  async updateDocument(userId: string, wid: string, documentId: string, input: unknown) {
    ...
    return db.transaction(async () => {
      ...
      const updated = await db.run(`UPDATE documents SET title=?,body=?,parentId=?,bodyRevision=?,updatedAt=?
        WHERE workspaceId=? AND id=? AND updatedAt=?`, data.title ?? previous.title, data.body ?? previous.body, parentId,
        bodyRevision, updatedAt, wid, documentId, data.expectedUpdatedAt)
  ...
  async deleteDocument(userId: string, wid: string, documentId: string) {
    return db.transaction(async () => {
      ...
      const pages = (await db.get<{ count: number }>('SELECT count(*) AS count FROM document_pages WHERE workspaceId=? AND documentId=?', wid, documentId))!.count
      await db.run('DELETE FROM documents WHERE workspaceId=? AND id=?', wid, documentId)
      await audit(userId, wid, 'document.delete', documentId, { unlinkedPages: pages })
      return { success: true, unlinkedPages: pages }
    })
  },
  ...
  async listPages(userId: string, wid: string, documentId: string): Promise<{ items: ItemWithSubtasks[]; total: number; truncated: boolean }> {
    ...
    await documentInWorkspace(wid, documentId)
    const total = (await db.get<{ count: number }>(`WITH RECURSIVE page_items(id) AS (
  ...
  async updateCommentReaction(userId: string, wid: string, documentId: string, commentId: string, input: unknown) {
    ...
    return db.transaction(async () => {
      ...
      const count = (await db.get<{ count: number }>('SELECT count(*) AS count FROM document_comment_reactions WHERE workspaceId=? AND documentId=? AND commentId=? AND emoji=?', wid, documentId, commentId, data.emoji))!.count

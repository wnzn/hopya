...
export type McpResult = { isError?: boolean; content: Array<{ type: 'text'; text: string }> }
export type McpRequest = (path: string, method?: string, body?: unknown) => Promise<McpResult> | McpResult

export function createMcpServer(request: McpRequest): McpServer {
  ...
  // This operator switch exposes mutations; the MCP host must still confirm each call with a human.

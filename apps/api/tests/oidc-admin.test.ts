...
async function login(f: Fixture, user: { id: string; email: string }) {
  await f.db.run('UPDATE users SET passwordHash=? WHERE id=?', passwordHash, user.id)
...
async function snapshot(f: Fixture) {
  const [users, identities, sessions, tokens, audits] = await Promise.all([
    f.db.all('SELECT * FROM users ORDER BY id'),
    f.db.all('SELECT * FROM oidc_identities ORDER BY id'),
    f.db.all('SELECT * FROM sessions ORDER BY id'),
    f.db.all('SELECT * FROM tokens ORDER BY id'),
    f.db.all('SELECT * FROM audit_logs ORDER BY id'),
  ])
  return {
    users,
    identities,
...
test('OIDC identity listing is admin-only, exact public metadata, strictly UUID-filtered and read-only', { timeout: 60000 }, async (t) => {
  ...
  const admin = await f.user(true); const target = await f.user(); const other = await f.user()
  const identities = [await link(f, admin.token, target.id), await link(f, admin.token, other.id), await link(f, admin.token, target.id)]
  await f.db.run('UPDATE users SET disabled=1 WHERE id=?', other.id)
...
test('OIDC administration denies credential theft, invalid IDs, disabled targets and cross-origin mutation without side effects', { timeout: 60000 }, async (t) => {
  ...
  assert.deepEqual(await snapshot(f), before)
  await f.db.run('UPDATE users SET isAdmin=0 WHERE id=?', admin.id)
...
test('OIDC unlink revokes every session and bearer token atomically, preserves other accounts and audits only safe details', { timeout: 60000 }, async (t) => {
  ...
  await f.db.run('INSERT INTO sessions (id,userId,tokenHash,expiresAt,createdAt) VALUES (?,?,?,?,?)', randomUUID(), target.id, 'expired-session-hash', '2000-01-01T00:00:00.000Z', now)
  await f.db.run('INSERT INTO tokens (id,userId,name,tokenHash,expiresAt,createdAt) VALUES (?,?,?,?,?,?)', randomUUID(), target.id, 'Expired', 'expired-token-hash', '2000-01-01T00:00:00.000Z', now)
  const before = await snapshot(f)
  const response = await f.request(`${path}/${identity.id}`, { method: 'DELETE', token: admin.token })
  assert.equal(response.status, 200, await response.clone().text())
  assert.deepEqual(await response.json(), { success: true })
  assert.deepEqual(await f.db.all('SELECT * FROM sessions WHERE userId=?', target.id), [])
  assert.deepEqual(await f.db.all('SELECT * FROM tokens WHERE userId=?', target.id), [])
  assert.deepEqual((await snapshot(f)).users, before.users)
  ...
  const audit = await f.db.all("SELECT actorId,workspaceId,resourceId,details FROM audit_logs WHERE action='admin.oidc.unlink'")
...
test('passwordless recovery requires another identity for the configured issuer, even when disabled', { timeout: 60000 }, async (t) => {
  ...
  for (const disabled of [0, 1]) {
    await f.db.run('UPDATE users SET disabled=? WHERE id=?', disabled, target.id)
    const before = await snapshot(f)
    const response = await f.request(`${path}/${identity.id}`, { method: 'DELETE', token: admin.token })
  ...
  assert.deepEqual(await f.db.all('SELECT id FROM tokens WHERE userId=?', target.id), [])
})
...
test('OIDC unlink rolls back identity, credentials and audit when the audit insertion fails', { timeout: 60000 }, async (t) => {
  ...
  const before = await snapshot(f)
  await f.db.run("CREATE TRIGGER fail_oidc_audit BEFORE INSERT ON audit_logs WHEN NEW.action='admin.oidc.unlink' BEGIN SELECT RAISE(ABORT, 'private audit failure'); END")
  try {
    const response = await f.request(`${path}/${identity.id}`, { method: 'DELETE', token: admin.token })
    assert.equal(response.status, 409)
    assert.deepEqual(await response.json(), { error: 'Operation conflicts with existing data' })
  } finally {
    await f.db.run('DROP TRIGGER fail_oidc_audit')
  }
  assert.deepEqual(await snapshot(f), before)
...
test('unlink during a verified token exchange cannot restore a session or JIT identity, including changed email', { timeout: 120000 }, async (t) => {
  for (const scenario of ['closed', 'jit-same-email', 'jit-changed-email', 'relinked', 'audit-rollback', 'unrelated-subject', 'unrelated-issuer']) {
    await t.test(scenario, async (t) => {
      ...
      const admin = await f.user(true); const target = await f.user(false, p.state.email)
      ...
      p.state.onToken = async () => {
        if (scenario === 'audit-rollback') await f.db.run("CREATE TRIGGER fail_oidc_audit BEFORE INSERT ON audit_logs WHEN NEW.action='admin.oidc.unlink' BEGIN SELECT RAISE(ABORT, 'audit failure'); END")
        try {
          ...
          if (scenario === 'audit-rollback') assert.deepEqual(await snapshot(f), before)
          else assert.deepEqual(await f.db.all('SELECT id FROM sessions WHERE userId=?', target.id), [])
        } finally {
          if (scenario === 'audit-rollback') await f.db.run('DROP TRIGGER fail_oidc_audit')
        }
      ...
      if (allowed) {
        ...
        assert.deepEqual(await f.db.all('SELECT id FROM sessions'), [])
        assert.deepEqual(await f.db.all("SELECT id FROM audit_logs WHERE action IN ('auth.sso.login','auth.sso.provision')"), [])
        if (scenario !== 'relinked') assert.deepEqual(await f.db.all('SELECT id FROM oidc_identities'), [])
        assert.equal((await f.request('/auth/me', { cookie })).status, 401)
      ...
      const audits = JSON.stringify((await snapshot(f)).audits)

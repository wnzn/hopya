...
import { connectNodes, deleteNode, insertNode, nodeLabel, publicHeaderNameError, removeEdges, runOutputRows, updateItemSupported, upstreamChoices } from "../lib/automation-graph";
...
function AutomationNodeCard({ data }: NodeProps<FlowNode>) {
  ...
  return (
    <article className={`automation-node automation-node-${node.type}${data.selected ? " selected" : ""}${data.errors.length ? " invalid" : ""}${data.previewed ? " previewed" : ""}`}>
      ...
      {node.type === "condition" ? <>
        <Handle id="true" type="source" position={Position.Right} style={{ top: "42%" }} aria-label="True branch" />
        <Handle id="false" type="source" position={Position.Right} style={{ top: "76%" }} aria-label="False branch" />
      </> : node.type !== "switch" && <Handle type="source" position={Position.Right} aria-label="Outgoing connection" />}
...
function Inspector({ graph, selectedId, catalog, credentials, members, errors, onChange, onSelect, onDelete, onAdd, onStatus }: {
  ...
  const branches = node.type === "condition" ? ["true", "false"] : node.type === "switch" ? [...cases.map((entry) => entry.branch), stringValue(config.defaultBranch)].filter(Boolean) : [];
  ...
  return <aside className="automation-inspector" aria-label="Selected node inspector">
    ...
    {node.type === "switch" && <fieldset className="bare-fieldset stack"><legend>Cases</legend>{cases.map((entry, index) => <div className="switch-case" key={index}><label>Branch name<input value={entry.branch} onChange={(event) => setConfig({ cases: cases.map((item, i) => i === index ? { ...item, branch: event.target.value } : item) })} /></label><ScalarEditor label={`Case ${index + 1} value`} value={entry.value} onChange={(value) => setConfig({ cases: cases.map((item, i) => i === index ? { ...item, value } : item) })} /><button type="button" onClick={() => setConfig({ cases: cases.filter((_, i) => i !== index) })}>Remove case</button></div>)}<button type="button" disabled={cases.length >= 20} onClick={() => setConfig({ cases: [...cases, { branch: `case-${cases.length + 1}`, value: "" }] })}>Add case</button><label>Default branch<input value={stringValue(config.defaultBranch)} onChange={(event) => setConfig({ defaultBranch: event.target.value })} /></label></fieldset>}
...
function GraphEditor({ wid, automation, catalog, credentials, members, onClose, onPublished }: { wid: string; automation: Automation; catalog: AutomationCatalog; credentials: AutomationCredential[]; members: Detail["members"]; onClose: () => void; onPublished: () => void }) {
  ...
  useEffect(() => {
    ...
    api<AutomationDraft>(`${base}/draft`, "GET", undefined, controller.signal).then((draft) => { setGraph(draft.graph); setRevision(draft.revision); setSelectedId(draft.graph.nodes.find((node) => node.type === "trigger")?.id ?? draft.graph.nodes[0]?.id ?? ""); }).catch((cause) => { if (!controller.signal.aborted) setError(message(cause)); });
    return () => controller.abort();
  }, [base]);
  const update = (next: AutomationGraph) => { setGraph(next); setDirty(true); setValidation(null); setPreview(null); };
  ...
  const requestClose = () => { if (!dirty || window.confirm("Close the builder and discard unsaved changes?")) onClose(); };
  ...
  return <Modal title={`Automation builder: ${automation.name}`} onClose={requestClose} focusFirstField={false} className="automation-editor-modal" heading={<div><h2>{automation.name}</h2><span className="draft-badge">Draft r{revision}</span>{dirty && <span className="dirty-badge">Unsaved changes</span>}</div>} headerActions={<><button type="button" disabled={busy || !dirty || publicHeaderIssues.length > 0} title={publicHeaderIssues.length ? "Fix sensitive public headers before saving" : undefined} onClick={() => void save()}>Save draft</button><button type="button" disabled={busy || publicHeaderIssues.length > 0} title={publicHeaderIssues.length ? "Fix sensitive public headers before validation" : undefined} onClick={() => void validate()}>Validate</button><button type="button" className="primary" disabled={busy || dirty || revision < 1} onClick={() => void publish()}>Publish</button></>}>
    <div className="automation-status" role="status" aria-live="polite">{status}</div><ErrorNotice error={error} />
    ...
    <div className="automation-builder">
      ...
      <section className={`automation-canvas panel-${panel}`} aria-label="Automation graph canvas"><ReactFlow nodes={flowNodes} edges={graph.edges} nodeTypes={nodeTypes} fitView minZoom={0.25} maxZoom={1.8} nodesFocusable edgesFocusable elementsSelectable deleteKeyCode={["Backspace", "Delete"]} onNodeClick={(_, node) => setSelectedId(node.id)} onNodesChange={(changes: NodeChange<FlowNode>[]) => { let next = graph; for (const change of changes) { if (change.type === "select" && change.selected) setSelectedId(change.id); if (change.type === "position" && change.position) next = { ...next, nodes: next.nodes.map((node) => node.id === change.id ? { ...node, position: change.position! } : node) }; if (change.type === "remove") next = deleteNode(next, change.id); } if (next !== graph) update(next); }} onEdgesChange={(changes: EdgeChange[]) => { const edgeIds = new Set(changes.filter((change) => change.type === "remove").map((change) => change.id)); if (edgeIds.size === 0) return; const next = removeEdges(graph, edgeIds); if (next === graph) { setStatus("Connection removal rejected because it would break a required branch or make a node unreachable."); return; } update(next); setStatus(`${edgeIds.size === 1 ? "Connection" : "Connections"} removed.`); }} onConnect={(connection: Connection) => { if (!connection.source || !connection.target) return; const next = connectNodes(graph, connection.source, connection.target, connection.sourceHandle ?? undefined); if (next === graph) setStatus("Connection rejected because it would create a cycle, replace a required branch, or orphan an existing path. Delete or restructure that path explicitly first."); else { update(next); setStatus("Nodes connected."); } }}><Background /><MiniMap pannable zoomable /><Controls showInteractive={false} /></ReactFlow></section>
...
export default function AutomationWorkspace({ detail, revision }: { detail: Detail; revision: number }) {
  ...
  return <><section className="settings-section" aria-labelledby="automations-heading"><div className="section-intro"><h2 id="automations-heading">Automations</h2><p>Build versioned event graphs with exclusive branches and explicit publishing. New automations start paused so setup cannot trigger accidental effects.</p></div><ErrorNotice error={error} /><ul className="record-list">{rows.map((automation) => { const count = automation.steps?.length ?? (automation.action ? 1 : 0); return <li key={automation.id}><div><strong>{automation.name}</strong><small className="muted">On {eventName(catalog, automation.event)} · {automation.graph ? "published graph" : `${count} legacy step${count === 1 ? "" : "s"}`} · version {automation.version}</small></div><div className="button-group"><span className={automation.enabled ? "count ok" : "count"}>{automation.enabled ? "Enabled" : "Paused"}</span><button type="button" disabled={busy} onClick={() => void mutate(() => api(`${base}/${automation.id}`, "PATCH", { enabled: !automation.enabled }))}>{automation.enabled ? "Pause" : "Resume"}</button><button type="button" disabled={!catalog} onClick={() => setEditing(automation)}>Edit graph</button><button type="button" disabled={busy} onClick={() => { if (window.confirm(`Queue a real test run for "${automation.name}"? Published actions may contact services or update the triggering task when event data permits.`)) void mutate(() => api(`${base}/${automation.id}/test`, "POST")).then((queued) => { if (queued) setRunRefresh((value) => value + 1); }); }}>Test run</button><button type="button" className="danger" disabled={busy} onClick={() => { if (window.confirm(`Delete automation "${automation.name}" and its run history?`)) void mutate(() => api(`${base}/${automation.id}`, "DELETE")); }}>Delete</button></div></li>; })}</ul>{!rows.length && <p className="muted">No automations yet.</p>}<button type="button" onClick={() => { setCreateError(""); setCreating(true); }}>New automation</button></section>
    ...
    {editing && catalog && <GraphEditor wid={wid} automation={editing} catalog={catalog} credentials={credentials} members={detail.members} onClose={() => setEditing(null)} onPublished={() => void refresh()} />}
  </>;
}

...
test('Docker initialization generates independent private secrets and keeps integrations/registration off', async (t) => {
  ...
  const postgresPassword = data.POSTGRES_PASSWORD
  assert.ok(postgresPassword)
  assert.match(postgresPassword, /^[a-f0-9]{64}$/)
  assert.equal(data.DATABASE_URL, `postgres://hopya:${postgresPassword}@postgres:5432/hopya`)
...
test('racing initializers publish exactly one complete configuration and leave no temporary secrets', async (t) => {
  ...
  assert.ok(data.APP_KEY && data.SETUP_TOKEN && data.POSTGRES_PASSWORD)
  assert.match(data.APP_KEY, /^[a-f0-9]{64}$/)
  assert.match(data.SETUP_TOKEN, /^[a-f0-9]{64}$/)
  assert.deepEqual((await readdir(root)).sort(), ['.env', 'data'])
})
